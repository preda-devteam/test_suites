...
default_filename="result/ERC20_history.log"
filename=${1:-$default_filename}
echo "run ERC20_history task" > $filename
...
for ((i=0;((2**i))<=$cores;i++));do
	...
	./$bin_file ./test_cases/ERC20_history.xtlts -order:$i -cores:$core_list -perf >> $filename
done
