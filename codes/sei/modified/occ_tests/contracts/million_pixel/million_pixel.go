...
func GetABI() []byte {
	...
	filePath := filepath.Join(currentDir, "MillionPixel.abi")
	...
	if err != nil || count > filesize {
		panic("failed to read MillionPixel contract ABI")
	}
	return bz[:count]
}
...
func GetBin() []byte {
	...
	currentDir := filepath.Dir(filename)
	filePath := filepath.Join(currentDir, "MillionPixel.bin")
	...
	if err != nil || count > filesize {
		panic("failed to read MillionPixel contract Bin")
	}
	bz, err := hex.DecodeString(string(code[:count]))
	if err != nil {
		panic("failed to decode MillionPixel contract binary")
	}
	cachedBin = bz
	return bz
}
