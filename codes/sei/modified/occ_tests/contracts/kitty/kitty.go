...
func GetABI() []byte {
	...
	filePath := filepath.Join(currentDir, "KittyBreeding.abi")
	...
	if err != nil || count > filesize {
		panic("failed to read KittyBreeding contract ABI")
	}
	return bz[:count]
}
...
func GetBin() []byte {
	...
	currentDir := filepath.Dir(filename)
	filePath := filepath.Join(currentDir, "KittyBreeding.bin")
	...
	if err != nil || count > filesize {
		panic("failed to read KittyBreeding contract Bin")
	}
	bz, err := hex.DecodeString(string(code[:count]))
	if err != nil {
		panic("failed to decode KittyBreeding contract binary")
	}
	cachedBin = bz
	return bz
}
