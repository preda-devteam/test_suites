...
func GetABI() []byte {
	...
	filePath := filepath.Join(currentDir, "Ballot.abi")
	...
	if err != nil || count > filesize {
		panic("failed to read Ballot contract ABI")
	}
	return bz[:count]
}
...
func GetBin() []byte {
	...
	currentDir := filepath.Dir(filename)
	filePath := filepath.Join(currentDir, "Ballot.bin")
	...
	if err != nil || count > filesize {
		panic("failed to read Ballot contract Bin")
	}
	bz, err := hex.DecodeString(string(code[:count]))
	if err != nil {
		panic("failed to decode Ballot contract binary")
	}
	cachedBin = bz
	return bz
}
