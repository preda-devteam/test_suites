...
func NewContext(testAccts []TestAcct, workers int, occEnabled bool) *TestContext {
	isCheckTx := false
	enableEVMCustomPrecompiles := true
	testApp := app.Setup(isCheckTx, enableEVMCustomPrecompiles, func(ba *baseapp.BaseApp) {
		ba.SetOccEnabled(occEnabled)
		ba.SetConcurrencyWorkers(workers)
	})
	ctx := testApp.BaseApp.NewContext(isCheckTx, tmproto.Header{Height: 1, ChainID: "sei-test", Time: time.Now()})
	ctx = ctx.WithContext(context.WithValue(ctx.Context(), dexutils.DexMemStateContextKey, testApp.MemState))
	bankkeeper := testApp.BankKeeper
	amounts := sdk.NewCoins(sdk.NewCoin("usei", sdk.NewInt(1000000000000000)))
...
func toTxBytes(testCtx *TestContext, msgs []*TestMessage) [][]byte {
	...
	for _, tm := range msgs {
		m := tm.Msg
		if !tm.IsEVM {
			for i := 0; i < len(testCtx.TestAccounts); i++ {
				if m.GetSigners()[0].Equals(testCtx.TestAccounts[i].AccountAddress) {
					priv = testCtx.TestAccounts[i].PrivateKey
					acct = testCtx.TestApp.AccountKeeper.GetAccount(testCtx.Ctx, testCtx.TestAccounts[i].AccountAddress)
					acc_idx = i
				}
			}
		}
		...
		tBuilder := tx.WrapTx(&txtype.Tx{
			...
			AuthInfo: &txtype.AuthInfo{
				Fee: &txtype.Fee{
					...
					Payer:    testCtx.TestAccounts[acc_idx].AccountAddress.String(),
					Granter:  testCtx.TestAccounts[acc_idx].AccountAddress.String(),
				},
			},
		})
