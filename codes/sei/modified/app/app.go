...
func (app *App) ExecuteTxsConcurrently(ctx sdk.Context, txs [][]byte, typedTxs []sdk.Tx, absoluteTxIndices []int) ([]*abci.ExecTxResult, sdk.Context) {
	...
	if ctx.IsOCCEnabled() {
		startPara := time.Now()
		res, ctx := app.ProcessTXsWithOCC(ctx, txs, typedTxs, absoluteTxIndices)
		if len(txs) > 0 {
			fmt.Println("tasks:", len(txs), "elapse:", time.Since(startPara).Seconds(), " parallel tps: ", int(float64(len(txs))/time.Since(startPara).Seconds()))
		}
		return res, ctx
	}
	startSeq := time.Now()
	...
	if len(txs) > 0 {
		fmt.Println("tasks:", len(txs), "elapse:", time.Since(startSeq).Seconds(), " sequential tps: ", int(float64(len(txs))/time.Since(startSeq).Seconds()))
	}
	return results, ctx
}
