...
function run_airdrop {
    ...
    for ((i=1; i<=$count; i++)); do
        ...
        taskset -c $cores cargo run --release -- airdrop --num-accounts 10000 --num-transactions 100000  >> $filename
        echo "" >> $filename
        echo "" >> $filename
        sleep 5s
    done
...
function run_ballot {
    ...
    for ((i=1; i<=$count; i++)); do
        cores=$(./../scripts/cpu_info.sh|cut -d ' ' -f$i)
        echo "============================== taskset in cpu: (${cores}) ==============================" >> $filename
        taskset -c $cores cargo run --release -- ballot-sharding --num-accounts 100000 --num-transactions 100000 --num-shardings $((2 ** ((i-1))))  >> $filename
        echo "" >> $filename
        echo "" >> $filename
        sleep 5s
    done
...
function run_erc20 {
    ...
    for ((i=1; i<=$count; i++)); do
        cores=$(./../scripts/cpu_info.sh|cut -d ' ' -f$i)
        echo "============================== taskset in cpu: (${cores}) ==============================" >> $filename
        taskset -c $cores cargo run --release -- param-sweep --num-accounts 10000 --block-sizes 100000  >> $filename
        echo "" >> $filename
        echo "" >> $filename
        sleep 5s
    done
...
function run_eth_historical {
    ...
    for ((i=1; i<=$count; i++)); do
        cores=$(./../scripts/cpu_info.sh|cut -d ' ' -f$i)
        echo "============================== taskset in cpu: (${cores}) ==============================" >> $filename
        taskset -c $cores  cargo run --release -- replay-erc20 --data-path "../data/ETH_2404_100000.csv"  >> $filename
        echo "" >> $filename
        echo "" >> $filename
        sleep 5s
    done
...
function run_kitty {
    ...
    for ((i=1; i<=$count; i++)); do
        cores=$(./../scripts/cpu_info.sh|cut -d ' ' -f$i)
        echo "============================== taskset in cpu: (${cores}) ==============================" >> $filename
        taskset -c $cores cargo run --release -- kitty --num-accounts 1001 --num-transactions 10000  >> $filename
        echo "" >> $filename
        echo "" >> $filename
        sleep 5s
    done
...
function run_million_pixel {
    ...
    for ((i=1; i<=$count; i++)); do
        cores=$(./../scripts/cpu_info.sh|cut -d ' ' -f$i)
        echo "============================== taskset in cpu: (${cores}) ==============================" >> $filename
        taskset -c $cores cargo run --release -- million-pixel --num-accounts 1001 --num-transactions 10000  >> $filename
        echo "" >> $filename
        echo "" >> $filename
        sleep 5s
    done
