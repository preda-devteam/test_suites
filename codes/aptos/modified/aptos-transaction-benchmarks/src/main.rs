...
use aptos_transaction_benchmarks::simulator::Similator;

use aptos_vm_logging::disable_speculative_logging;
use clap::{Parser, Subcommand};
use proptest::prelude::*;
use std::{
    error::Error, fs::File, net::SocketAddr, time::{SystemTime, UNIX_EPOCH}
};
...
pub fn read_csv_with_header(file_path: &str) -> Result<Vec<(usize, usize)>, Box<dyn Error>> {
...
fn param_sweep(opt: ParamSweepOpt) {
    ...
    let block_sizes = opt.block_sizes.unwrap_or_else(|| vec![100000]);
...
fn replay_erc20(opt: ReplayERC20HistoricOpt){
    disable_speculative_logging();

    let concurrency_level = num_cpus::get();
    let mut bencher = Similator::with_account_nums(opt.num_accounts);
    let mut par_measurements: Vec<usize> = Vec::new();
    let mut seq_measurements: Vec<usize> = Vec::new();
    ...
    let total_runs = opt.num_runs + opt.num_warmups;
    //load historical data
    for _i in 0..total_runs{
        let historical_data_for_erc20 = read_csv_with_header(&opt.data_path).unwrap();
        let transactions = bencher.gen_transaction_for_erc20(historical_data_for_erc20);
        let (tps1,tps2) = bencher.execute_blockstm_benchmark(
            transactions, 
            run_parallel, run_sequential, 
            concurrency_level, 
            maybe_block_gas_limit
        );
        par_measurements.push(tps1);
        seq_measurements.push(tps2);
    }
    println!("\nconcurrency_level = {}\n", concurrency_level);
    let mut seq_tps = 0;
    if run_sequential {
        println!("Sequential TPS: {:?}", seq_measurements);
        let mut seq_sum = 0;
        for m in &seq_measurements {
            seq_sum += m;
        }
        seq_tps = seq_sum / seq_measurements.len();
        println!("Avg Sequential TPS = {:?}", seq_tps,);
    }

    if run_parallel {
        println!("Parallel TPS: {:?}", par_measurements);
        let mut par_sum = 0;
        for m in &par_measurements {
            par_sum += m;
        }
        let par_tps = par_sum / par_measurements.len();
        println!("Avg Parallel TPS = {:?}", par_tps,);
        if run_sequential {
...
fn airdrop(opt:CommonOpt){
    disable_speculative_logging(); 
    let mut bencher = Similator::with_account_nums(opt.num_accounts);
    bencher.run_airdrop(opt.num_transactions);
}

fn ballot(opt:CommonOpt){
    disable_speculative_logging();
    let mut bencher = Similator::with_account_nums(opt.num_accounts);
    bencher.run_ballot(opt.num_transactions);
}

fn ballot_sharding(opt:CommonShardingOpt){
    disable_speculative_logging();
    let mut bencher = Similator::with_account_nums(opt.num_accounts);
    bencher.run_ballot_sharding(opt.num_transactions,opt.num_shardings);
}

fn kitty(opt:CommonOpt){
    disable_speculative_logging();
    let mut bencher = Similator::with_account_nums(opt.num_accounts);
    bencher.run_kitty(opt.num_transactions);
}

fn mp(opt:CommonOpt){
    disable_speculative_logging();
    let mut bencher = Similator::with_account_nums(opt.num_accounts);
    bencher.run_mp(opt.num_transactions);
}

fn empty(opt:CommonOpt){
    disable_speculative_logging();
    let mut bencher = Similator::with_account_nums(opt.num_accounts);
    bencher.run_empty(opt.num_transactions);
}
...
fn main() {
    ...
    match args.command {
        ...
        BenchmarkCommand::ReplayERC20(opt) =>replay_erc20(opt),
        BenchmarkCommand::Airdrop(opt) => airdrop(opt),
        BenchmarkCommand::Ballot(opt) => ballot(opt),
        BenchmarkCommand::BallotSharding(opt) => ballot_sharding(opt),
        BenchmarkCommand::Kitty(opt)=>kitty(opt),
        BenchmarkCommand::MillionPixel(opt)=>mp(opt),
        BenchmarkCommand::Empty(opt)=>empty(opt),
