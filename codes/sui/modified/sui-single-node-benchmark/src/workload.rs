...
impl Workload {
    ...
    pub fn is_test_kitty(&self) -> bool {
        self.workload_kind.is_test_kitty()
    }
        ...
        match &self.workload_kind {
            WorkloadKind::PTB {
                ...
                let shared_objects = ctx
                    ...
                    .await;
                let kitty_objects = ctx
                    .prepare_kitty_objects(move_package.0, *test_kitty)
                    .await;
                let land_objects = ctx
                    .prepare_land_objects(move_package.0, *num_land_objects).await;
