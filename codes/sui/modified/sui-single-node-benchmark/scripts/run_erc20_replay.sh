...
for ((i=1; i<=$count; i++)); do
    ...
    taskset -c $cores cargo run --release --bin sui-single-node-benchmark -- --account-path "./../data/USDT_100k_addr_count.csv" --erc20-tx-path "./../data/USDT_240101_240331_data_100000.csv" --component with-tx-manager ptb --erc20-transfer >> $filename
    echo "" >> $filename
    echo "" >> $filename
    sleep 3s
done
