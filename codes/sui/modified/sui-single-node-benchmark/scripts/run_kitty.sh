...
for ((i=1; i<=$count; i++)); do
    ...
    taskset -c $cores cargo run --release --bin sui-single-node-benchmark -- --tx-count 100000 --component with-tx-manager ptb --test-kitty  >> $filename
    echo "" >> $filename
    echo "" >> $filename
    sleep 3s
done
...
parallel_tps_data=($(cat $filename | grep "Execution kitty all finished"|tr -d '\r' | awk -F '=' '{print int($2)}'))
