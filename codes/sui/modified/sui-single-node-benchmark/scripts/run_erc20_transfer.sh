#!/bin/bash
default_filename="./result/erc20_transfer.log"
...
for ((i=1; i<=$count; i++)); do
    ...
    taskset -c $cores cargo run --release --bin sui-single-node-benchmark -- --tx-count 100000 --component with-tx-manager ptb --num-transfers 1  >> $filename
    echo "" >> $filename
    echo "" >> $filename
    sleep 3s
done
