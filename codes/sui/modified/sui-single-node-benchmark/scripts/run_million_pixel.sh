...
echo "run million pixel tasks" > $filename

for ((i=1; i<=$count; i++)); do
    cores=$(./cpu_info.sh|cut -d ' ' -f$i)
    echo "============================== taskset in cpu: (${cores}) ==============================" >> $filename
    taskset -c $cores cargo run --release --bin sui-single-node-benchmark -- --tx-count 100000 --component with-tx-manager ptb --num-land-objects 1 >> $filename
    echo "" >> $filename
    echo "" >> $filename
    sleep 3s
done
