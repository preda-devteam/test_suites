#!/bin/bash
default_filename="./result/ballot_sharding.log"
filename=${1:-$default_filename}
count=$(./cpu_info.sh | awk '{print NF}')

echo "run ballot sharding tasks" > $filename

for ((i=1; i<=$count; i++)); do
    cores=$(./cpu_info.sh|cut -d ' ' -f$i)
    echo "============================== taskset in cpu: (${cores}) ==============================" >> $filename
    taskset -c $cores cargo run --release --bin sui-single-node-benchmark -- --tx-count 100000 --component with-tx-manager ptb --num-shared-objects $((2 ** ((i-1))))  >> $filename
    echo "" >> $filename
    echo "" >> $filename
    sleep 3s
done
