...
module move_benchmark::benchmark {
    ...
    // erc20 transfer
    #[allow(lint(self_transfer))]
    public fun split_and_transfer_coin(coin: &mut Coin<SUI>, to:address, amount:u64, ctx: &mut TxContext) {
        let coin_for_transfer = coin.split(amount, ctx);
        transfer::public_transfer(coin_for_transfer, to);
    }
}
